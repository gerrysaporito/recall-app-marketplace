...
interface CreateBotAppForm {
  appId: string;
  fields: Record<string, string>;
}

export function BotAppsCreateDialog({
  open,
  onOpenChange,
  botId,
}: {
  ...
  const createBotApp = useMutation({
    mutationFn: async (data: CreateBotAppForm) => {
      const response = await fetch(`/api/bots/${botId}/apps`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          appId: data.appId,
          botAppDataFields: Object.entries(data.fields).map(([key, value]) => ({
            key,
            value,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Select an app and configure its fields for this bot.
          </DialogDescription>
        </DialogHeader>
